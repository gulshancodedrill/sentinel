...
var HierarchicalPie = function(options) {
  ...
  jQuery.extend(config, config, options || {});
  ...
  this.tabulateCategories = function (data) {
    ...
    var rowTemplate = Mustache.compile(jQuery('#rowTemplate').html());
    ...
    var tableRows = tbody.selectAll("tr")
        ...
        .enter()
          ...
          .attr('style', function (d) {
            return d[config.dataSchema.valueField] == 0 ? ' display:none' : '';
          })
          .html(function(d) {
  ...
  this.init = function() {
    self.palette = d3.scale.ordinal()
      .range(['#3FB648', '#ED2322', '#FFDA00', '#A6CE38', '#FCB03F', '#B01B52', '#91268F', '#006EB9', '#2AC1E3']);
    ...
    self.svg = d3.select(config.chartId).append("svg")
      ...
      .append("g")
        .attr("transform", "translate(" + self.radius + "," + ((self.height / 2) + 30) + ")");

    self.focusGroup = self.svg.append('g').attr('class', 'focus-group');

    self.percentLabel = self.focusGroup.append('g')
      .attr('class', 'arc-percent').append("text").attr("dy", "0.3em");

    self.labelField = self.focusGroup.append('g')
      .attr('class', 'arc-cost').append("text")
      .attr("transform", "translate(0," + (-(self.height / 2) - 20) + ")");

    self.navigation = jQuery(config.navigation);
  ...
  this.pieMouseOver = function (d, i) {
    ...
    self.labelField.text(d.data['category']);
