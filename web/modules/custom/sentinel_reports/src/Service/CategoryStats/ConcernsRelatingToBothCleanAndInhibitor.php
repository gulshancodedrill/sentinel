...
class ConcernsRelatingToBothCleanAndInhibitor extends CategoryStatsBase {
  ...
  public function setQuery(): void {
    ...
    $this->query = "
    SELECT
      CONCAT(COUNT(CASE WHEN not_cleaned_flushes >= 1 AND inhibitor_low >= 1 AND unknown_inhib = 0 THEN 1 END), '#', GROUP_CONCAT(CASE WHEN not_cleaned_flushes >= 1 AND inhibitor_low >= 1 AND unknown_inhib = 0 THEN pid END SEPARATOR '+')) AS needed_more_thorough_flushing_and_needed_more_inhibitor,
      CONCAT(COUNT(CASE WHEN not_cleaned_flushes >= 1 AND (not_enough_circulation >= 1 OR not_enough_circulation2 >= 1) AND unknown_inhib = 0  THEN 1 END), '#', GROUP_CONCAT(CASE WHEN not_cleaned_flushes >= 1 AND (not_enough_circulation >= 1 OR not_enough_circulation2 >= 1) AND unknown_inhib = 0 THEN pid END SEPARATOR '+')) AS needed_more_thorough_flushing_and_inhibitor_needed_more_thorough_circulation,
      CONCAT(COUNT(CASE WHEN not_cleaned >= 1 AND inhibitor_low >= 1 AND unknown_inhib = 0  THEN 1 END), '#', GROUP_CONCAT(CASE WHEN not_cleaned >= 1 AND inhibitor_low >= 1 AND unknown_inhib = 0  THEN pid END SEPARATOR '+')) AS needed_more_thorough_cleaning_and_needed_more_inhibitor,
      CONCAT(COUNT(CASE WHEN not_cleaned >= 1 AND (not_enough_circulation >= 1 OR not_enough_circulation2 >= 1) AND unknown_inhib = 0 THEN 1 END), '#', GROUP_CONCAT(CASE WHEN not_cleaned >= 1 AND (not_enough_circulation >= 1 OR not_enough_circulation2 >= 1) AND unknown_inhib = 0 THEN pid END SEPARATOR '+')) AS needed_more_thorough_cleaning_and_inhibitor_needed_more_thorough_circulation,
      CONCAT(COUNT(CASE WHEN unknown_inhib > 0 THEN 1 END), '#', GROUP_CONCAT(CASE WHEN unknown_inhib > 0 THEN pid END SEPARATOR '+')) AS A_non_Sentinel_inhibitor_was_used
    FROM (
      SELECT
        pid,
        COUNT(CASE WHEN ic.field_stat_individual_comment_value LIKE '%Unknown inhibitor%' THEN 1 END) AS unknown_inhib,
        COUNT(CASE WHEN sr.field_stat_recommendation_value LIKE '%clean%' AND sr.field_stat_recommendation_value NOT LIKE '%Adequate protection can only be confirmed by the manufacturer of the detected inhibitor%' THEN 1 END) AS not_cleaned,
        COUNT(CASE WHEN ic.field_stat_individual_comment_value LIKE '%X100 inhibitor is too low%' THEN 1 END) AS inhibitor_low,
        COUNT(CASE WHEN sr.field_stat_recommendation_value LIKE '%System flush required%' THEN 1 END) as not_cleaned_flushes,
        ...
        COUNT(CASE WHEN ic.field_stat_individual_comment_value LIKE '%X100 level very high%' THEN 1 END) AS not_enough_circulation2
        FROM (
          SELECT
            pid,
            COUNT(CASE WHEN field_stat_element_name_value = 'X100' AND (td.name = 'Fail' OR td.name = 'Warning') THEN 1 END) AS x100_fail,
            COUNT(CASE WHEN en.field_stat_element_name_value != 'X100' AND td.name = 'Fail' THEN 1 END) AS other_fail_results
          FROM
          field_data_field_stat_pack_reference_id rid
